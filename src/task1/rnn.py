...
from src.task1.model_rnn import SeqLabelRNN
...
def _build_model(config: dict, meta: dict, device) -> SeqLabelRNN:
    model = SeqLabelRNN(
        config=config,
...
def train(config: dict):
    print("[task1_rnn] Starting training...")
    ...
    print("[task1_rnn] Training done.")


def evaluate(config: dict):
    print("[task1_rnn] Starting evaluation...")
    ...
    if ckpt_path and os.path.isfile(ckpt_path):
        print(f"[task1_rnn] Loading local checkpoint: {ckpt_path}")
        load_checkpoint(ckpt_path, model, device=str(device))
    elif config.get("hf_repo_id"):
        print(f"[task1_rnn] Local checkpoint not found at '{ckpt_path}'.")
        print(f"[task1_rnn] Attempting to pull from HuggingFace: {config['hf_repo_id']}")
        ...
        filename  = os.path.basename(ckpt_path) if ckpt_path else "rnn_best.pt"
        try:
            pulled = pull_from_hub(config["hf_repo_id"], filename, local_dir=local_dir)
            load_checkpoint(pulled, model, device=str(device))
            print(f"[task1_rnn] Loaded checkpoint from HuggingFace.")
        except Exception as e:
            raise FileNotFoundError(
                f"\n\n[task1_rnn] Could not load checkpoint.\n"
        ...
        raise FileNotFoundError(
            f"[task1_rnn] No checkpoint found at '{ckpt_path}' and no hf_repo_id configured.\n"
    ...
    print("[task1_rnn] Validation metrics:", metrics)
    save_metrics(metrics, out_dir, filename="rnn_metrics.json")

    # ---- External test file ----
    test_file = config.get("test_file", None)
    if test_file and os.path.isfile(test_file):
        print(f"[task1_rnn] Running inference on test file: {test_file}")
        ...
        with torch.no_grad():
            for start in range(0, len(tensors), batch_size):
                ...
                for idx, row in enumerate(preds):
                    # For external files, we might just output exactly the length predicted
                    actual_length = batch_l[idx].item()
                    test_preds.append(indices_to_string(row[:actual_length], idx2plain, pad_idx))

        pred_path = os.path.join(out_dir, "task1_rnn.txt")
        with open(pred_path, "w", encoding="utf-8") as f:
            for line in test_preds:
                f.write(line + "\n")
        print(f"[task1_rnn] Predictions written to {pred_path}")
    else:
        pred_path = os.path.join(out_dir, "task1_rnn.txt")
        ...
        with open(pred_path, "w", encoding="utf-8") as f:
            for pred, tgt in samples:
                ...
                f.write(f"PRED: {pred}\n")
                f.write("-" * 60 + "\n")
        print(f"[task1_rnn] 5 sample predictions written to {pred_path}")

    print("[task1_rnn] Evaluation done.")
