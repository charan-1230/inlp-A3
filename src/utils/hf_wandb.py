...
def push_to_hub(
    ...
    token = token or os.environ.get("HF_TOKEN_PUSH")
    if not token:
        raise EnvironmentError(
            "HuggingFace push token not found. "
            "Set HF_TOKEN_PUSH in your .env file."
        )
    api = HfApi()
...
def pull_from_hub(
    ...
    token = token or os.environ.get("HF_TOKEN_READ")
    if not token:
        raise EnvironmentError(
            "HuggingFace read token not found. "
            "Set HF_TOKEN_READ in your .env file."
        )
